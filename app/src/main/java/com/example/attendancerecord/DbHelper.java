...
class DbHelper extends SQLiteOpenHelper {
    ...
    long addClass(String className,String subjectName){
        ...
        value.put(CLASS_NAME_KEY,className);
        value.put(SUBJECT_NAME_KEY,subjectName);

        return database.insert(CLASS_TABLE_NAME,null,value);
    }
    ...
    long updateClass(long cid,String className,String subjectName){
        ...
        return database.update(CLASS_TABLE_NAME,value,C_ID+"=?",new String[]{String.valueOf(cid)});
    }
    ...
    long updateStudent(long sid,String name){
        SQLiteDatabase database=this.getWritableDatabase();
        ContentValues value=new ContentValues();
        value.put(SUBJECT_NAME_KEY,name);

        return database.update(STUDENT_TABLE_NAME,value,S_ID+"=?",new String[]{String.valueOf(sid)});
    }
    ...
    long addStatus(long sid,long cid,String date,String status){
        ...
        value.put(S_ID,sid);
        value.put(C_ID,cid);
        value.put(DATE_KEY,date);
        value.put(STATUS_KEY,status);
        return database.insert(STATUS_TABLE_NAME,null,value);
    }
    ...
    long updateStatus(long sid,String date,String status){
        ...
        return database.update(STATUS_TABLE_NAME,value,whereClause,null);
    }
    ...
    Cursor getDistinctMonths(long cid){
        SQLiteDatabase database=this.getReadableDatabase();
        return database.query(STATUS_TABLE_NAME,new String[]{DATE_KEY},C_ID+"="+cid,null,"substr("+DATE_KEY+",4,7)",null,null);//01.04.2021
    }
}
